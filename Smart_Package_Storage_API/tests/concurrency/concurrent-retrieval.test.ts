...
let db: PgDatabase;
  ...
  it('allows only one concurrent retrieval to collect the package', async () => {
    const owner = await createCustomer(client, {
    ...
    const storage = new DefaultPackageStorageService(
      db,
    ...
    const retrieval = new DefaultPackageRetrievalService(
      db,
      lockers,
      customers,
      packages,
        ...
        retrieval.retrievePackage(
          {
            ...
            receivedByUsername: owner.username,
          } as any,
          {
            idempotencyKey: idempotencyKey(),
            requestHash: `retrieve-concurrent-${i}`,
    ...
    expect(packageResult.rows[0].status).toBe('COLLECTED');
    ...
    expect(lockerResult.rows[0].is_occupied).toBe(false);
    ...
    expect(chargesResult.rows[0].count).toBe(1);
  });
});