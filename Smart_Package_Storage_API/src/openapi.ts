...
export const openApiDocument = {
  ...
  components: {
    ...
    schemas: {
      ...
      StorePackageRequest: { type: 'object', additionalProperties: false, required: ['customerId', 'widthCm', 'heightCm', 'breadthCm', 'weightGrams', 'hasFragileItems', 'packageName'], properties: { customerId: { type: 'string', format: 'uuid' }, packageName: { type: 'string' },  widthCm: { type: 'integer', minimum: 1 }, heightCm: { type: 'integer', minimum: 1 }, breadthCm: { type: 'integer', minimum: 1 }, weightGrams: { type: 'integer', minimum: 1 }, hasFragileItems: { type: 'boolean' } } },
      ...
      RetrievePackageResponse: { type: 'object', required: ['packageId', 'collectedAt', 'heldTimeMs', 'chargedAmountCents', 'walletBalanceCents', 'lockerReleased','packageName'], properties: { packageId: { type: 'string', format: 'uuid' }, collectedAt: { type: 'string', format: 'date-time' }, heldTimeMs: { type: 'integer', minimum: 0 }, chargedAmountCents: { type: 'integer', minimum: 0 }, walletBalanceCents: { type: 'integer', minimum: 0 }, lockerReleased: { type: 'boolean', enum: [true] }, packageName: { type: 'string' } } },
